...
function SurveyContent<T extends BaseSurveyResponse>({
  ...
  const isFormValid = (): boolean => {
    ...
    const allRatingQuestionsAnswered = questions.every(
      (question) =>
    ...
    const singleOpenEndedValid =
      !singleOpenEndedQuestion ||
      (responses[singleOpenEndedQuestion.field] &&
        meetsWordRequirement(
          responses[singleOpenEndedQuestion.field] as string
        ));

    // Check if all multiple open-ended questions meet word requirement
    const multipleOpenEndedValid =
      !openEndedQuestions ||
      openEndedQuestions.every(
        (question) =>
    ...
    return (
      allRatingQuestionsAnswered &&
      singleOpenEndedValid &&
      multipleOpenEndedValid
    );
  };
                  ...
                  <Box key={index}>
                    ...
                    <Textarea
                      ...
                      onChange={
                        isCompleted
                          ? undefined
                          : (e) => onTextChange(question.field, e.target.value)
                      }
                      placeholder={isCompleted ? "" : "Type here..."}
                      autosize
                      minRows={5}
                      ...
                      styles={{
                        input: {
                          ...
                          border: isCompleted
                            ...
                            : responses[question.field] &&
                              !meetsWordRequirement(
                                responses[question.field] as string
                              )
                            ? "2px solid #e53e3e"
                            : "none",
                          borderRadius: "10px",
                    ...
                    {responses[question.field] &&
                      !meetsWordRequirement(
                        ...
                        >
                          Please provide at least 15 words. Current word count:{" "}
                          {countWords(responses[question.field] as string)}
                        </Text>
                      )}
                  </Box>
                ))}
